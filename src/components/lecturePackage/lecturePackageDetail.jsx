...
const LecturePackageDetail = observer(() => {
  ...
  useEffect(() => {
    const fetchLecturePackage = async () => {
      ...
      try {
        const response = await axiosClient.get("/packages/detail", {
        ...
        const responseCount = await axiosClient.get("/packages/lecturecount", {
          params: { lecturePackageId },
        });
        ...
        const profileResponse = await axiosClient.get(
          `/packages/profile?nickname=${teacherNickname}`
        );
  ...
  useEffect(() => {
    const increaseViewCount = async (authorNickname) => {
      ...
      if (authorNickname && nickname) {
        ...
        console.log(
          `isViewed: ${isViewed}, authorNickname: ${authorNickname}, nickname: ${nickname}`
        );

        if (authorNickname === nickname) {
          if (!isViewed) {
            document.cookie = `${viewCookieName}=true; path=/; max-age=${
              60 * 60 * 24
            }`;
            await axiosClient.put(`/packages/view/${lecturePackageId}`, null, {
              params: { nickname: encodedNickname },
            });
  ...
  const isAuthor =
    lecturePackage && authStore.getNickname() === lecturePackage.nickname;
  ...
  return (
    <div className={styles.bodys}>
                ...
                <div className={styles.topInfo}>
                  <div className={styles.infoItem}>
                    <p className={styles.register}>
                      등록 날짜: {lecturePackage.registerDate}
                    </p>
                  </div>
                </div>
                <div>
                  <div className={styles.redBox}>
                    <div
                      ...
                      style={{
                        backgroundColor: lecturePackage.backgroundColor,
                      }}
                  ...
                  {lecturePackage.learningContent && (
                    <div className={styles.learningPerson}>
                      <div className={styles.sectionHeader}>
                        <img
                          src="/images/learning_person.png"
                          alt="학습대상자 아이콘"
              ...
              <div className={styles.field1}>
                <div class="field1-header">
                  ...
                  <div className={styles.categories}>
                    {lecturePackage.subCategoryName
                      .split(",")
                      .map((category, index) => (
            ...
            <div className={styles.applyBox}>
              <div className={styles.fixedBox}>
                ...
                <div className={styles.price}>
                  {formatPrice(lecturePackage.priceForever)}원
                </div>
                {authStore.getNickname() === lecturePackage.nickname &&
                authStore.checkIsTeacher() === true ? (
                  <span className={styles.applyButton}>수강신청 버튼</span>
                ) : (
                  <button
                    className={styles.applyButton}
                    onClick={
                      isInCart ? () => router.push("/cart") : handleApply
                    }
                  >
                ...
                <div>
                  ...
                  <span className={styles.level}>
                    {lecturePackage.averageClassLength}
                  </span>
                </div>
                <div className={styles.infoItem}>
      ...
      {isModalOpen && (
        <ProfileModal profile={profile} onClose={closeProfileModal} />
      )}
    </div>
  );
});
